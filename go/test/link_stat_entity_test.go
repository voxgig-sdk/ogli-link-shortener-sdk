...
func TestLinkStatEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.LinkStat(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.LinkStat(nil).Stream("list", nil, nil) {
...
func link_statBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OGLI_LINK_SHORTENER_TEST_LINK_STAT_ENTID")
