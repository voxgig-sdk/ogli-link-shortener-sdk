...
func TestLinkDirect(t *testing.T) {
	t.Run("direct-list-link", func(t *testing.T) {
		setup := linkDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-link", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-link", func(t *testing.T) {
		setup := linkDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-link", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type linkDirectSetupResult struct {
	client *sdk.OgliLinkShortenerSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func linkDirectSetup(mockres any) *linkDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"OGLI_LINK_SHORTENER_TEST_LINK_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["OGLI_LINK_SHORTENER_TEST_LINK_ENTID"]; ok {
		...
		return &linkDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &linkDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
