...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OGLILINKSHORTENER_TEST_LIVE") == "TRUE" ||
		os.Getenv("OGLILINKSHORTENER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OGLILINKSHORTENER_TEST_EXPLAIN"); explain != "" {
