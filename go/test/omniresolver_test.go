...
import (
	...
	omni "github.com/voxgig-sdk/ogli-link-shortener-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OgliLinkShortenerError); is {
				out := map[string]any{
					"name":    "OgliLinkShortenerError",
...
func omniCtx(arg any, client *sdk.OgliLinkShortenerSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OgliLinkShortenerSDK); is {
			client = live
