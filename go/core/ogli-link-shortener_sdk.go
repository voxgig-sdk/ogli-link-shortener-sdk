...
type OgliLinkShortenerSDK struct {
	Mode     string
...
func (sdk *OgliLinkShortenerSDK) OptionsMap() map[string]any {
...
func (sdk *OgliLinkShortenerSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *OgliLinkShortenerSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *OgliLinkShortenerSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OgliLinkShortenerSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *OgliLinkShortenerSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *OgliLinkShortenerSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("OgliLinkShortenerSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *OgliLinkShortenerSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OgliLinkShortenerSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("OgliLinkShortenerSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *OgliLinkShortenerSDK {
	...
	sdk := NewOgliLinkShortenerSDK(sdkopts)
	sdk.Mode = "test"
