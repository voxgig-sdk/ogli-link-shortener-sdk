...
boru-driven command-line client **and** interactive REPL for the OgliLinkShortener
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ogli-link-shortener-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ogli-link-shortener-cli list link
   ```
...
```sh
./ogli-link-shortener-cli load 1 link          # scalar shorthand for {id:1}
./ogli-link-shortener-cli load '{id:1}' link     # explicit match map
```
...
```sh
./ogli-link-shortener-cli update '{id:1,name:"new"}' link
```
...
Run with no arguments to open a REPL (prompt `ogli-link-shortener>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `link`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ogli-link-shortener-cli`. |
...
(`./ogli-link-shortener-cli <expr>`) and interactively (the REPL), and expressions compose the
