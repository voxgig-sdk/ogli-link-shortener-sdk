...
describe("LinkDirect", function()
  it("should direct-list-link", function()
    local setup = link_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-link", setup.live and "live" or "unit")
  ...
  it("should direct-load-link", function()
    local setup = link_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-link", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function link_direct_setup(mockres)
