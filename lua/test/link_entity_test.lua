...
describe("LinkEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Link(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Link(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    -- CREATE
    local link_ref01_ent = client:Link(nil)
    local link_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local link_ref01_match = {}

    local link_ref01_list_result, err = link_ref01_ent:list(link_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(link_ref01_list_result)
    ...
    -- REMOVE
    local link_ref01_match_rm0 = {
      id = link_ref01_data["id"],
    }
    local _, err = link_ref01_ent:remove(link_ref01_match_rm0, nil)
    assert.is_nil(err)

    -- LIST
    local link_ref01_match_rt0 = {}

    local link_ref01_list_rt0_result, err = link_ref01_ent:list(link_ref01_match_rt0, nil)
    assert.is_nil(err)
    assert.is_table(link_ref01_list_rt0_result)
