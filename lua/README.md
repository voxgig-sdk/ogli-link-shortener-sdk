...
The Lua SDK for the OgliLinkShortener API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Link(nil):list(nil, nil)
...
```lua
local result, err = client:Link(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
```lua
-- Create
local created, _ = client:Link(nil):create({ name = "Example" }, nil)

-- Update
client:Link(nil):update({ id = created["id"], name = "Example-Renamed" }, nil)

-- Remove
client:Link(nil):remove({ id = created["id"] }, nil)
```
...
local result, err = client:OgliLinkShortener(nil):load(
  { id = "test01" }, nil
)
