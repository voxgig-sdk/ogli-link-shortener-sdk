...
local LinkStatEntity = {}
LinkStatEntity.__index = LinkStatEntity


function LinkStatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LinkStatEntity)
  self._name = "link_stat"
  self._client = client
...
function LinkStatEntity:get_name()
  return self._name
end


function LinkStatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LinkStatEntity.new(self._client, opts)
end
...
function LinkStatEntity:mark_deleted()
  self._deleted = true
end


function LinkStatEntity:deleted()
  return true == self._deleted
end


function LinkStatEntity:data_set(args)
  if args ~= nil then
...
function LinkStatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LinkStatEntity:match_set(args)
  if args ~= nil then
...
function LinkStatEntity:match_get()
...
function LinkStatEntity:stream(action, args, callopts)
...
function LinkStatEntity:_run_op(ctx, post_done)
  local utility = self._utility
