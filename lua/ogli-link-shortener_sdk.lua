...
require("ogli-link-shortener_types")
...
local OgliLinkShortenerSDK = {}
OgliLinkShortenerSDK.__index = OgliLinkShortenerSDK
...
OgliLinkShortenerSDK._make_feature = _make_feature


function OgliLinkShortenerSDK.new(options)
  local self = setmetatable({}, OgliLinkShortenerSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OgliLinkShortenerSDK:options_map()
...
function OgliLinkShortenerSDK:get_utility()
  return Utility.copy(self._utility)
end


function OgliLinkShortenerSDK:get_root_ctx()
  return self._rootctx
end


function OgliLinkShortenerSDK:prepare(fetchargs)
...
function OgliLinkShortenerSDK:direct(fetchargs)
...
function OgliLinkShortenerSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function OgliLinkShortenerSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "OgliLinkShortenerSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function OgliLinkShortenerSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OgliLinkShortenerSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OgliLinkShortenerSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:LinkStat():list() / client:LinkStat():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function OgliLinkShortenerSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OgliLinkShortenerSDK.new(sdkopts)
  sdk.mode = "test"
