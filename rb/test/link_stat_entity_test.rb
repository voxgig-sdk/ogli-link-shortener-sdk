...
class LinkStatEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.LinkStat(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.LinkStat(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = link_stat_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "link_stat." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    link_stat_ref01_data = nil
    if link_stat_ref01_data_raw.length > 0
      link_stat_ref01_data = Helpers.to_map(link_stat_ref01_data_raw[0][1])
    end

    # LIST
    link_stat_ref01_ent = client.LinkStat(nil)
...
def link_stat_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "link_stat", "LinkStatTestData.json")
