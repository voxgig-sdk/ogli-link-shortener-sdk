...
module OgliLinkShortenerUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OgliLinkShortenerResponse.new({})
    ...
    if fetch_err
      ...
      response = OgliLinkShortenerResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OgliLinkShortenerResponse.new(fetched)
    else
