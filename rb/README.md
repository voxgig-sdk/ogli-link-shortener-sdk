# OgliLinkShortener Ruby SDK

The Ruby SDK for the OgliLinkShortener API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Link(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
```ruby
# Create
created, _ = client.Link(nil).create({ "name" => "Example" }, nil)
...
result, err = client.OgliLinkShortener(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── OgliLinkShortener_sdk.rb       -- Main SDK module
...
The main module (`OgliLinkShortener_sdk`) exports the SDK class
