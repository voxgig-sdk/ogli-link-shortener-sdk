...
class OgliLinkShortenerSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OgliLinkShortenerHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OgliLinkShortenerHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OgliLinkShortenerFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OgliLinkShortenerUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = OgliLinkShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OgliLinkShortenerHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OgliLinkShortenerError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OgliLinkShortenerHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OgliLinkShortenerError.new(
        "graphql_error", "OgliLinkShortenerSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OgliLinkShortenerSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
