...
//    only - a typed OgliLinkShortenerContext there would make every ctx
...
final class OgliLinkShortenerOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OgliLinkShortenerContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OgliLinkShortenerSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OgliLinkShortenerResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OgliLinkShortenerError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OgliLinkShortenerResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OgliLinkShortenerError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OgliLinkShortenerError(is_string($code) ? $code : '', $msg);
    }
