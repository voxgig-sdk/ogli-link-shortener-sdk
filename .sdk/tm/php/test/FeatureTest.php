...
// OgliLinkShortener SDK feature test
//
...
class FtCtrl extends OgliLinkShortenerControl
{
...
class FtClient
{
    ...
    private OgliLinkShortenerUtility $utility;

    public function __construct(OgliLinkShortenerUtility $utility, array $options)
    {
    ...
    public function get_utility(): OgliLinkShortenerUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OgliLinkShortenerContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OgliLinkShortenerContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OgliLinkShortenerContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OgliLinkShortenerConfig::make_feature($name);
    ...
    private static function buildUrl(OgliLinkShortenerSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OgliLinkShortenerError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OgliLinkShortenerResponse($fetched);
        }
    ...
    private function fail(OgliLinkShortenerContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OgliLinkShortenerContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
