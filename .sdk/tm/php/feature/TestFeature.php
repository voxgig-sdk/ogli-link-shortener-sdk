...
// OgliLinkShortener SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OgliLinkShortenerTestFeature extends OgliLinkShortenerBaseFeature
{
    ...
    public function init(OgliLinkShortenerContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OgliLinkShortenerContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OgliLinkShortenerParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OgliLinkShortenerContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OgliLinkShortenerContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OgliLinkShortenerParam::call($ctx, $k);
