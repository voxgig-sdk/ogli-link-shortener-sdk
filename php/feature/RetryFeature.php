...
// OgliLinkShortener SDK retry feature
...
class OgliLinkShortenerRetryFeature extends OgliLinkShortenerBaseFeature
{
    ...
    public function init(OgliLinkShortenerContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OgliLinkShortenerContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OgliLinkShortenerContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OgliLinkShortenerContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
