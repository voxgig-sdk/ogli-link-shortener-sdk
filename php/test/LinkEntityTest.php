...
class LinkEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = OgliLinkShortenerSDK::test($seed, null);
        $seen = iterator_to_array($base->Link(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = OgliLinkShortenerConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = OgliLinkShortenerSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Link(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OGLI_LINK_SHORTENER_TEST_LINK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $link_ref01_data = Helpers::to_map(is_object($link_ref01_data_result) && method_exists($link_ref01_data_result, 'data_get') ? $link_ref01_data_result->data_get() : $link_ref01_data_result);
        ...
        $link_ref01_markdef_up0_name = "createdAt";
        ...
        $link_ref01_resdata_up0 = Helpers::to_map(is_object($link_ref01_resdata_up0_result) && method_exists($link_ref01_resdata_up0_result, 'data_get') ? $link_ref01_resdata_up0_result->data_get() : $link_ref01_resdata_up0_result);
        ...
        $link_ref01_data_dt0_load_result = Helpers::to_map(is_object($link_ref01_data_dt0_loaded) && method_exists($link_ref01_data_dt0_loaded, 'data_get') ? $link_ref01_data_dt0_loaded->data_get() : $link_ref01_data_dt0_loaded);
...
function link_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OGLI_LINK_SHORTENER_TEST_LINK_ENTID");
