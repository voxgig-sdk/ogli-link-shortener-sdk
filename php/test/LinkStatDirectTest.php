...
class LinkStatDirectTest extends TestCase
{
    public function test_direct_list_link_stat(): void
    {
        $setup = link_stat_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-link_stat", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["link_stat01"] as $_liveKey) {
