...
class OgliLinkShortenerSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OgliLinkShortenerHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OgliLinkShortenerHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OgliLinkShortenerFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OgliLinkShortenerFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OgliLinkShortenerUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OgliLinkShortenerHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OgliLinkShortenerHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OgliLinkShortenerHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OgliLinkShortenerError($op . "_allow",
                "OgliLinkShortenerSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OgliLinkShortenerHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OgliLinkShortenerError("graphql_error",
                "OgliLinkShortenerSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new OgliLinkShortenerSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
