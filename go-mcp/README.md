# ogli-link-shortener-mcp

[MCP](https://modelcontextprotocol.io) server exposing the OgliLinkShortener SDK as
two agent tools — `ogli-link-shortener_list` and `ogli-link-shortener_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ogli-link-shortener-mcp)
make build
...
claude mcp add --scope user ogli-link-shortener \
  -- /absolute/path/to/ogli-link-shortener-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ogli-link-shortener-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ogli-link-shortener_list: first page of records
{ "entity": "link" }
{ "entity": "link", "query": { } }

// ogli-link-shortener_load: one record by id
{ "entity": "link", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ogli-link-shortener-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ogli-link-shortener \
     -- "$PWD"/dist/*/ogli-link-shortener-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ogli-link-shortener_list` and `ogli-link-shortener_load` tools now appear
   in new sessions. Ask the agent to *"list link using ogli-link-shortener"*
   and it calls `ogli-link-shortener_list` with `{"entity":"link"}`.
...
```sh
export OGLI_LINK_SHORTENER_APIKEY=sk_live_xxx            # API key
export OGLI_LINK_SHORTENER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ogli-link-shortener_list` tool
...
### Call the `ogli-link-shortener_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ogli-link-shortener_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ogli-link-shortener_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OGLI_LINK_SHORTENER_APIKEY` | API key sent with every request. |
| `OGLI_LINK_SHORTENER_BASE` | Optional override of the API base URL. |
...
```sh
./ogli-link-shortener-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ogli-link-shortener_load","arguments":{"entity":"link","query":{"id":1}}}}'
```
