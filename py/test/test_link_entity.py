...
class TestLinkEntity:
    ...
    def test_should_stream(self):
        ...
        base = OgliLinkShortenerSDK.test(seed, None)
        seen = list(base.Link(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from oglilinkshortener_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = OgliLinkShortenerSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Link(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OGLI_LINK_SHORTENER_TEST_LINK_ENTID JSON to run live")
        client = setup["client"]
        ...
        link_ref01_data = helpers.to_map(runner.entity_data(link_ref01_ent.create(link_ref01_data, None)))
        ...
        link_ref01_resdata_up0 = helpers.to_map(runner.entity_data(link_ref01_ent.update(link_ref01_data_up0_up, None)))
        ...
        link_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(link_ref01_data_dt0_loaded))
...
def _link_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("OGLI_LINK_SHORTENER_TEST_LINK_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("OGLI_LINK_SHORTENER_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("OGLI_LINK_SHORTENER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("OGLI_LINK_SHORTENER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
