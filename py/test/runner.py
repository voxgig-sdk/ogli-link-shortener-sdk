# OgliLinkShortener SDK test runner
...
class OgliLinkShortenerTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OgliLinkShortenerTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OgliLinkShortenerTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OgliLinkShortenerTestRunner.getenv("OGLI_LINK_SHORTENER_TEST_LIVE")
        override = OgliLinkShortenerTestRunner.getenv("OGLI_LINK_SHORTENER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OgliLinkShortenerTestRunner.getenv(key)
        ...
        explain = OgliLinkShortenerTestRunner.getenv("OGLI_LINK_SHORTENER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if OgliLinkShortenerTestRunner._test_control is not None:
            return OgliLinkShortenerTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OgliLinkShortenerTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OgliLinkShortenerTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OgliLinkShortenerTestRunner.load_test_control()
...
def load_env_local():
    OgliLinkShortenerTestRunner.load_env_local()


def env_override(m):
    return OgliLinkShortenerTestRunner.env_override(m)


def entity_data(v):
    return OgliLinkShortenerTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OgliLinkShortenerTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OgliLinkShortenerTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OgliLinkShortenerTestRunner.load_test_control()


def live_client_options():
    return OgliLinkShortenerTestRunner.live_client_options()


def live_delay_ms():
    return OgliLinkShortenerTestRunner.live_delay_ms()
