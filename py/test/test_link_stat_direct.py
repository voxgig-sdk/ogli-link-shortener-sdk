...
class TestLinkStatDirect:

    def test_should_direct_list_link_stat(self):
        setup = _link_stat_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-link_stat", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["link_stat01"]:
...
def _link_stat_direct_setup(mockres):
