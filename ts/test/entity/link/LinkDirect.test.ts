...
describe('LinkDirect', async () => {
  ...
  test('direct-load-link', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-link', setup.live)) return
  ...
  test('direct-list-link', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-link', setup.live)) return
    const { client, calls } = setup
