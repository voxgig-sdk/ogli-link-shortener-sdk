...
describe('LinkStatDirect', async () => {
  ...
  test('direct-list-link_stat', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-link_stat', setup.live)) return
    if (skipIfMissingIds(t, setup, ["link_stat01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OGLI_LINK_SHORTENER_TEST_LINK_STAT_ENTID']
