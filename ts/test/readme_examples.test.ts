...
//     (\`new OgliLinkShortenerSDK(...)\` / \`OgliLinkShortenerSDK.test(...)\` become
//     \`OgliLinkShortenerSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OgliLinkShortenerSDK'
...
const TEST_SEED = {"entity":{"link":{"test01":{"id":"test01"}},"link_stat":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OgliLinkShortenerSDK(...)\` and any
// \`OgliLinkShortenerSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OgliLinkShortenerSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OgliLinkShortenerSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
