...
(0, node_test_1.describe)('LinkStatEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when OGLI_LINK_SHORTENER_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('OGLI_LINK_SHORTENER_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.OgliLinkShortenerSDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['list']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'link_stat.' + op, live))
                return;
        }
        ...
        let link_stat_ref01_data = Object.values(setup.data.existing.link_stat)[0];
        // LIST
        const link_stat_ref01_ent = client.LinkStat();
        const link_stat_ref01_match = {};
        link_stat_ref01_match['link_id'] = setup.idmap['link01'];
        const link_stat_ref01_list = (await link_stat_ref01_ent.list(link_stat_ref01_match)).map((e) => e.data());
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/link_stat/LinkStatTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.OgliLinkShortenerSDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['link_stat01', 'link_stat02', 'link_stat03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
    ...
    if (live) {
        client = new __1.OgliLinkShortenerSDK(merge([
...
//# sourceMappingURL=LinkStatEntity.test.js.map